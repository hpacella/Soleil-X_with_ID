...
do
  ...
  for m = 0, MAX_ANGLES_PER_QUAD do
    [(function() local __quotes = terralib.newlist() for q = 1, 8 do __quotes:insert(rquote
...
local function mkBound(wall)
  ...
  do
    ...
    for idx in [faces[1]] do
      ...
      if epsw < 1.0 then
        [(function() local __quotes = terralib.newlist() for _,q in ipairs(incomingQuadrants) do __quotes:insert(rquote
          for m = 0, quadrantSize(q, num_angles) do
            value +=
      ...
      [(function() local __quotes = terralib.newlist() for _,q in ipairs(outgoingQuadrants) do __quotes:insert(rquote
        for m = 0, quadrantSize(q, num_angles) do
          if [terralib.newlist{
