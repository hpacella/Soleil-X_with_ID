...
function Exports.emitArrayReduce(dims, op, lhs, rhs)
  ...
  return rquote
    var tmp = [rhs];
    [(function() local __quotes = terralib.newlist() for i = 0,dims-1 do
      if     op == '+'   then __quotes:insert(rquote lhs[i] +=   tmp[i] end)
      elseif op == '-'   then __quotes:insert(rquote lhs[i] -=   tmp[i] end)
      elseif op == '*'   then __quotes:insert(rquote lhs[i] *=   tmp[i] end)
      elseif op == '/'   then __quotes:insert(rquote lhs[i] /=   tmp[i] end)
      elseif op == 'max' then __quotes:insert(rquote lhs[i] max= tmp[i] end)
      elseif op == 'min' then __quotes:insert(rquote lhs[i] min= tmp[i] end)
      else assert(false) end
    end return __quotes end)()];
  end
end
